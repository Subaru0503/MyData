...
void SceneGame::Load()
{
	...
	// ファイル読み込み
	ifstream file("Assets/Map/Stage.csv");
	...
	if (!file)
	{
		MessageBox(NULL, "Stage.csvファイルが読み込めませんでした", "SceneGame.cpp", MB_OK);
		return;
	}
	...
	// ファイル読み込み
	file.open("Assets/TargetScrew/Stage1TargetScrew.csv");

	// ファイルが読み込めてるか
	if (!file)
	{
		MessageBox(NULL, "Stage1TargetScrew.csvファイルが読み込めませんでした", "SceneGame.cpp", MB_OK);
		return;
	}
	...
	if (!file)
	{
		MessageBox(NULL, "UI.csvファイルが読み込めませんでした", "SceneGame.cpp", MB_OK);
		return;
	}
	...
	// ファイル読み込み
	file.open("Assets/Time/TimeParam.csv");	// ファイルオープン
	...
	// ファイル読み込み
	file.open("Assets/UI/Pause/Pause.csv");	// ファイルオープン
...
// プレイヤー作成
void SceneGame::PlayerCreate(ifstream& file, string cell)
{
	//----変数宣言----
...
// 地面作成
void SceneGame::GroundCreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// ゴール作成
void SceneGame::GoalCreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// ネジ作成
void SceneGame::ScrewCreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// ギミック作成
void SceneGame::GimmickCreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// ギミックに対象のネジを設定
void SceneGame::SetTargetScrew(ifstream & file, string cell)
{
	//----変数宣言----
...
// 時間UI作成
void SceneGame::TimeCreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// 通常UI作成(特に特有の動きがないUI)
void SceneGame::InGameNormalUICreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// ポーズUI作成
void SceneGame::PauseMoziUICreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// 通常UI作成(チュートリアル画面
void SceneGame::MenuNormalUICreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// 左矢印UI作成
void SceneGame::LeftUICreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// 右矢印UI作成
void SceneGame::RightUICreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// ゲームスタートUI作成
void SceneGame::StartUICreate(ifstream & file, string cell)
{
	//----変数宣言----
...
// ゲームオーバー後に表示するUIを作成
void SceneGame::GameOverUICreate()
{	
	// スカイドーム取得
	SkyDome* skydome = GetObj<SkyDome>("SkyDome");
...
// クリアタイムを表示するUIを作成
void SceneGame::ClearTimeUICreate()
{
	//----クリアUI生成----
