...
// ========== コンストラクタ ==========
CSceneStage3::CSceneStage3(CSceneMng* pSceneMng, CSoundMng* pSoundMng)
	: CSceneStageBase(pSceneMng, pSoundMng)
{
	m_pShopMenu = new CShopMenu(3, m_pUI, m_pSoundMng);
	m_pTimer = new CTimer(240);
	// マップ読み込み
	...
	if (!m_pStageModel[2]->Load("Assets/Model/Stage/Stage3/Stage3.fbx", 1.0f, Model::Flip::ZFlip)) {
		MessageBox(NULL, "Assets/Model/Stage/Stage/stage3/Stage3.fbx", "Error", MB_OK);
	}
	//m_pStageModel[1]->SetPixelShader(ShaderList::GetPS(ShaderList::PS_LAMBERT));
...
// ========== Update関数 =============
void CSceneStage3::Update(float tick)
{
#if DEBUG_KEY
	// ----- 強制シーン遷移 -----
	...
	if (!m_start) {
		...
		if (m_pSoundMng->CheckSoundEnd(CSoundMng::TimeSE::countDown1))		// カウントダウンが鳴りやんだら
		{
			...
			m_pSoundMng->playSound(CSoundMng::BGM::game3);					// BGM再生
...
void CSceneStage3::StartCameraWork()
{
	if (m_flg)
	{
		// ----- 変数宣言 -----
...
// 陸・海・空を描画する
void CSceneStage3::DrawStage(DirectX::XMFLOAT4X4 *mat, VertexShader* vs)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		case 2:	// 陸
			SetRenderTargets(1, &pRTV, pDSV);
			//---変換行列を計算
			world = DirectX::XMMatrixTranspose(DirectX::XMMatrixScaling(1.04, 1.02, 1.04) *
				DirectX::XMMatrixTranslation(0.0f, 0.65f, 0.0f));
			break;
		}
...
// ======================================
void CSceneStage3::LoadMap(list<Object*>* pObjList, list<CCollisionArea*>* pAreaList, CPlayer* pPlayer, CObjectMng* pObjMng)
{
	// =-=-= 準備 =-=-=
	// ----- 変数宣言 -----
	string line;		// 行ごとの文字列
	...
	// ----- マップデータ読み込み -----
	ifstream file("Assets/map/STAGE3.csv");
	if (!file.is_open()) {
		MessageBox(NULL, "Stage3が読み込めない", "Error", MB_OK);
	}
	...
	for (int i = 0; i < maxLine; i++)	// 全ての行を読み込み
	{
		...
		case 1:	// プレイヤー
			switch (height)	{
			case 0: y = STAGE3_HT0;	break;
			case 1: y = STAGE3_HT1;	break;
			case 2: y = STAGE3_HT2;	break;
			}
		...
		case 6:		// 地面
			...
			break;
		case 7:		// 高台
			pAreaList->push_back(new CAreaHill(x, y, z, sx, sy, sz, attr, 3));
			break;
		case 8:		// 坂
			pAreaList->push_front(new CAreaSlope(height, x, y, z, sx, sy, sz, direct, attr, 3));
			break;
		case 9:		// チョコの木
			pObjList->push_back(new ChocolateTree(x, y, z, sx, sy, sz, level));
			break;
		case 10:	// キャンディ岩
			pObjList->push_back(new CandyRock(x, y, z, sx, sy, sz, level));
			break;
		case 11:	// お菓子の家
			pObjList->push_back(new SweetsHouse(x, y, z, sx, sy, sz));
			break;
		case 12:	// チョコの破片(木材)
			pObjList->push_back(new ChocolatePiece(x, y, z, sx, sy, sz));
			break;
		case 13:	// キャンディーの破片(石)
			pObjList->push_back(new CandyPiece(x, y, z, sx, sy, sz));
			break;
		case 14:	// フェンス
			pObjList->push_back(new ChocolateFence(x, y, z, sx, sy, sz, 90.0f * (direct - 1), level));
			break;
		case 15:	// ろうそく
			pObjList->push_back(new Candle(x, y, z, sx, sy, sz, level));
			break;
		case 16:	// ドーナツ
			pObjList->push_back(new Donuts(x, y, z, sx, sy, sz));
			break;
		case 17:	// ケーキ
			pObjList->push_back(new CakePiece(x, y, z, sx, sy, sz));
			break;
		case 18:	// イチゴ
			pObjList->push_back(new Strawberry(x, y, z, sx, sy, sz));
			break;
		case 19:	// アポロ
			pObjList->push_back(new Apolo(x, y, z, sx, sy, sz));
			break;
		case 20:	// クッキー
			pObjList->push_back(new Cookie(x, y, z, sx, sy, sz));
			break;
		case 21:	// クラッカー
			switch (height) {
			case 0: y = STAGE3_HT0;	break;
			...
			pObjList->push_back(new Biscuit(x, y - sy / 2, z, sx, sy, sz, y));
			break;
		}
	}
