...
Shader::~Shader()
{
	...
	for (ID3D11ShaderResourceView* ptemp : m_tempSRVs)
	{
		if (ptemp != nullptr)
		{
			ptemp->Release();
...
void PixelShader::Bind(void)
{
	...
	if (!m_Flg)
	{
		m_tempSRVs.resize(m_pTextures.size(), nullptr);
		pContext->PSGetShaderResources(0, m_pTextures.size(), m_tempSRVs.data());
		m_Flg = 1;
	}

	for (int i = 0; i < m_pTextures.size(); ++i)
	{
		if (m_tempSRVs[i] != nullptr)
		{
			pContext->PSSetShaderResources(i, 1, &m_pTextures[i]);
			m_tempSRVs[i]->Release();
			m_tempSRVs[i] = nullptr;
		}
	}
}
