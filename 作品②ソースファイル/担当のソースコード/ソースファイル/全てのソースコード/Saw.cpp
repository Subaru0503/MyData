	...
	,SawCount(0.0f)
{
	// ----- モデル -----
	m_pModel = new Model;
	if (!m_pModel->Load("Assets/Model/saw/nokogiri_taiki.fbx", 0.17f, Model::Flip::XFlip, Model::Kind::Anime)) {
		MessageBox(NULL, "Assets/Model/saw/nokogiri_taiki.fbx", "Error", MB_OK);
	}
	...
	anime[0] = m_pModel->AddAnimation("Assets/Model/saw/nokogiri_taiki.fbx");
	anime[1] = m_pModel->AddAnimation("Assets/Model/saw/nokogiri.fbx");
}
...
void CSaw::Draw(DirectX::XMFLOAT4X4 *mat, VertexShader* vs)
{
...
void CSaw::SetPlayerPos(DirectX::XMFLOAT3 pos)
{
	m_pos = pos;
}

void CSaw::SawAnimetion()
{
	SawCount = 60.0f;
}
