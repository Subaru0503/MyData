...
void ShaderList::SetMaterial(const Model::Material& material)
{
	...
	//param[2].x = 0.1f; //カスタムパーツ表示のせいでてかる
	//param[2].y = 0.1f;
	//param[2].z = 0.1f;
...
//=====上書き対策用セットマテリアル=====
void ShaderList::SetMaterial_Custom(const Model::Material & material)
{
	DirectX::XMFLOAT4 param[3] = {
		material.diffuse,
		material.ambient,
...
float4 main(PS_IN pin) : SV_TARGET
{
    ...
    dotNL = lerp(shadowFactor, 1.0f, saturate(dotNL));

	float ambientStrength = 2.0f;
	
	float3 diffuse = objDiffuse.rgb * lightDiffuse.rgb * ambientStrength;
    float3 ambient = objAmbient.rgb * lightDiffuse.rgb;
    float3 specular = objSpecular.rgb * lightDiffuse.rgb * 0.01f;
    color.rgb *= saturate(diffuse * dotNL + ambient);
