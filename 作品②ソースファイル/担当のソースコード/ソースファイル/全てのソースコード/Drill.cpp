	...
	, DrillCount(0.0f)
{
	// ----- モデル -----
	m_pModel = new Model;
	if (!m_pModel->Load("Assets/Model/drill/DrilArm_taiki.fbx", 0.17f, Model::Flip::XFlip,Model::Kind::Anime)) {
		MessageBox(NULL, "Assets/Model/drill/DrilArm_taiki.fbx", "Error", MB_OK);
	}
	...
	anime[0] = m_pModel->AddAnimation("Assets/Model/drill/DrilArm_taiki.fbx");
	anime[1] = m_pModel->AddAnimation("Assets/Model/drill/DrilArm.fbx");
}
...
void CDrill::Draw(DirectX::XMFLOAT4X4 *mat, VertexShader* vs)
{
...
void CDrill::SetPlayerPos(DirectX::XMFLOAT3 pos)
{
	m_pos = pos;
}

void CDrill::DrillAnimetion()
{
	DrillCount = 60.0;
}
