...
// ========== コンストラクタ ==========
CSceneStage1::CSceneStage1(CSceneMng* pSceneMng, CSoundMng* pSoundMng)
	: CSceneStageBase(pSceneMng, pSoundMng)
{
	m_pShopMenu = new CShopMenu(2, m_pUI, m_pSoundMng);
	m_pTimer = new CTimer(300);
	// マップ読み込み
	...
	m_pCollision->SetTutorialIcon(m_pTutorial);
	// オブジェクトへのモデル割り当て
	...
	if (!m_pStageModel[2]->Load("Assets/Model/Stage/Stage1/Stage1.fbx", 1.0f, Model::Flip::ZFlip)) {
		MessageBox(NULL, "Assets/Model/Stage/Stage/stage1/Stage1.fbx", "Error", MB_OK);
	}
...
// ========== Update関数 ==========
void CSceneStage1::Update(float tick)
{
#if DEBUG_KEY
	// ----- 強制シーン遷移 -----
	...
	// =-=-= チュートリアル =-=-=
	if (m_pPlayer->GetTutorial()) {

		// ***チュートリアルUIの更新処理

		if (IsKeyTrigger('E') || state.Gamepad.wButtons&XINPUT_GAMEPAD_B)	m_pPlayer->SetTutorialDraw(false);
...
void CSceneStage1::Draw()
{
	...
	if (m_pPlayer->GetTutorial()) {
		// **** チュートリアル画面の描画処理
		m_pShopBG->Draw();
		m_pTutorial->Draw(m_pPlayer->GetTutorialNum());
		//m_pPause->Draw();
	}
...
void CSceneStage1::StartCameraWork()
{
	if (m_flg)
	{
		// ----- 変数宣言 -----
...
// 陸・海・空を描画する
void CSceneStage1::DrawStage(DirectX::XMFLOAT4X4 *mat, VertexShader* vs)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		case 2:	// 陸
			...
			//---変換行列を計算
			world = DirectX::XMMatrixTranspose(DirectX::XMMatrixScaling(3.1, 3.1, 2.7) *
				DirectX::XMMatrixTranslation(-1.0f, 0.0f, 0.8f));
			break;
		}
...
// 戻り値：なし
// ～関数概要～
// マップ1を読み込んでデータを各オブジェクトに設定する
// ======================================
void CSceneStage1::LoadMap(list<Object*>* pObjList, list<CCollisionArea*>* pAreaList, CPlayer* pPlayer, CObjectMng* pObjMng)
{
	// =-=-= 準備 =-=-=
	// ----- 変数宣言 -----
	string line;		// 行ごとの文字列
	...
	// ----- マップデータ読み込み -----
	ifstream file("Assets/map/STAGE1.csv");
	if (!file.is_open()) {
		MessageBox(NULL, "Stage1が読み込めない", "Error", MB_OK);
	}
	...
	for (int i = 0; i < maxLine; i++)	// 全ての行を読み込み
	{
		...
		case 1:	// プレイヤー
			switch (height) {
			case 0: y = STAGE1_HT0;	break;
			case 1: y = STAGE1_HT1;	break;
			case 2: y = STAGE1_HT2;	break;
			}
		...
		case 6:		// 地面
			...
			break;
		case 7:		// 高台
			pAreaList->push_back(new CAreaHill(x, y, z, sx, sy, sz, attr, 1));
			break;
		case 8:		// 坂
			pAreaList->push_front(new CAreaSlope(height, x, y, z, sx, sy, sz, direct, attr, 1));
			break;
		case 9:		// 木
			pObjList->push_back(new CTree(x, y - 0.2f/*根っこをめり込ませるための補正*/, z, sx, sy - 0.4f, sz, level));
			break;
		case 10:	// 岩
		...
		case 14:	// フェンス
			...
			break;
		case 15:	// チュートリアル看板
			pObjList->push_back(new CTutorialSignboard(x, y, z, sx, sy, sz, 90.0f * (direct - 1), attr - 1));
			m_pTutorial->SetPlayer(m_pPlayer);
			break;
		}
	}
